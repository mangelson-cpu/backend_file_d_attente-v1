...
For this specific project, the database structure (`structure_seule.sql`) and seed data (`volumes/db/init/data.sql`) are mounted directly in `docker-compose.yml`. 
This means that on a **fresh installation**, when you run `docker compose up -d` for the first time, these scripts are executed automatically after the Supabase internal schema is ready. You do NOT have to run the huge `backup_db.sql` unless you specifically want the old data.
